...
describe('memory cache', () => {
  ...
  const nextInteger1 = memoize((base = 1): number => base + getNextInteger());
  const nextInteger2 = memoizeFactory({ maxCacheSizePerTarget: 10, cacheDuration: -1 })(
    (base = 1): number => base + getNextInteger()
  );
  const nextInteger3 = memoizeFactory({ cacheDuration: 200 })((base = 1): number => base + getNextInteger());
